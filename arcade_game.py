...
class SpaceShooter(arcade.Window):
    ...
    def setup(self):
        ...
        self.game_timer = 0.0
        self.timer_display = arcade.Text(
            "0.0",
            self.width - 100 * SCALING,
            self.height - 40,
            arcade.color.BISQUE,
            24,
            100,
        ...
        # Add Level to screen
        self.level = 1
        self.level_display = arcade.Text(
            "1",
            self.width - 100 * SCALING,
            self.height - 67,
            arcade.color.BISQUE,
    ...
    def add_life(self):
        life = arcade.Sprite("images/a_shield_round_mithril.png", SCALING * 0.5)
    ...
    def on_update(self, delta_time: float):
        ...
        # Did we hit a shield?
        if (
            collided_powerups := self.player.collides_with_list(self.powerups)
        ) and not self.player.collides_with_list(self.clouds_list):
        ...
        if (
            collided_enemies := self.player.collides_with_list(self.enemies_list)
        ) and not self.player.collides_with_list(self.clouds_list):
            self.remove_life()
